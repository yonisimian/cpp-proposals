...
Given a structured binding declaration with assignment markers:
...
```cpp
int x;
auto __e = get_pair();              // hidden variable holding the result
x = std::get<0>(std::move(__e));    // assignment to existing variable
auto&& y = std::get<1>(std::move(__e)); // declaration of new binding
```

### Formal Definition

For a structured binding declaration of the form:

```
auto [&id₁, id₂, &id₃, ...] = expr;
```

Where some identifiers are marked (with `&`) and others are not:

1. **Unmarked identifiers** behave exactly as in current structured bindings — they introduce new bindings.

2. **Marked identifiers** must refer to variables already declared in an enclosing scope. The corresponding element of the decomposed object is assigned to this variable using `operator=`.

### Example Transformations

The following table shows the proposed syntax and its semantic equivalent:

| Proposed Syntax | Semantic Equivalent |
|-----------------|---------------------|
| `auto [&x, y] = p;` | `auto __e = p; x = get<0>(__e); auto&& y = get<1>(__e);` |
| `auto& [&x, y] = p;` | `auto& __e = p; x = get<0>(__e); auto& y = get<1>(__e);` |
| `auto&& [&x, y] = f();` | `auto&& __e = f(); x = get<0>(move(__e)); auto&& y = get<1>(move(__e));` |
| `auto [&x, &y] = p;` | `auto __e = p; x = get<0>(__e); y = get<1>(__e);` |
