...
Extend structured binding syntax to optionally use the `&` symbol for elements in the _sb-identifier-list_ inside the binding list to denote a binding to an existing variable.
...
This proposal intentionally focuses on a single, well-defined extension: allowing assignment to existing variables within structured bindings. Several related features are explicitly **out of scope** for this proposal but may be addressed in future work.

### Out of Scope: Per-Element Attributes

Allowing attributes on individual elements within structured bindings:

```cpp
auto [[[maybe_unused]] x, y] = get_pair();  // NOT proposed here
```

This is a valuable extension but orthogonal to the assignment feature proposed here.

### Out of Scope: Per-Element cv-qualifiers

Allowing different cv-qualifiers on individual elements:

```cpp
auto [const x, y] = get_pair();  // NOT proposed here
```

While useful, this significantly expands the scope and grammar changes required.

### Out of Scope: Structured Binding Packs

Interaction with structured binding packs (variadic structured bindings):

```cpp
auto [...xs] = get_tuple();  // pack expansion in structured bindings
auto [&first, ...&rest] = get_tuple();  // assignment to pack - NOT proposed here
```

The interaction between this proposal and structured binding packs requires careful consideration and is deferred to future work.

### Rationale for Narrow Scope

Keeping this proposal focused provides several benefits:

1. **Simpler specification** — Fewer grammar changes and edge cases to consider
2. **Easier implementation** — Compiler vendors can implement with lower risk
3. **Faster adoption** — A focused proposal can progress through the committee more efficiently
4. **Extensibility** — The syntax chosen here should not preclude future extensions

The features listed above can be proposed separately as pure extensions once this foundational capability is established.
