...
## Open Question: Outer cv-qualifiers

The interaction between outer cv-qualifiers and assignment markers requires further discussion:

```cpp
const auto [&x, y] = get_pair();  // What should this mean?
```

Possible interpretations:

1. **Ill-formed** — Cannot assign to `x` because the binding is const
2. **`const` applies only to new declarations** — `x` is assigned (not const), `y` is const

This proposal does not prescribe a specific behavior and welcomes committee input on the preferred semantics. A conservative approach would be to make such combinations ill-formed initially, allowing future proposals to relax this restriction if desired.
